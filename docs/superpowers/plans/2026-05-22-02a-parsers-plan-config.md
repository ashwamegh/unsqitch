...
describe('parsePlanFile', () => {
  ...
  it('preserves entry order in entries array', () => {
    ...
    expect(types).toEqual(['pragma', 'pragma', 'pragma', 'change', 'change', 'tag', 'change', 'change']);
  });
...
```typescript
import type { PlanFile, PlanEntry, PlanChange, PlanTag, PlanEntryType } from '../types/plan';

const PRAGMA_RE = /^%([\w-]+)=(.+)$/;
const TAG_RE = /^@([\w-]+)\s+(\S+)\s+([^<]*)<([^>]+)>\s*(?:#\s*(.+))?$/;
const CHANGE_RE = /^([\w-]+)\s*(?:\[([^\]]+)\])?\s+(\S+)\s+([^<]*)<([^>]+)>\s*(?:#\s*(.+))?$/;
