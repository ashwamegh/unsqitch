...
const ipcStub = {
  ...
  settingsSet: vi.fn().mockResolvedValue({ success: true }),
  projectTargets: vi.fn().mockResolvedValue({ targets: [], defaultTarget: undefined }),
  sqitchDetect: vi.fn().mockResolvedValue({
    found: true,
    path: "/usr/bin/sqitch",
    version: "1.4.1",
    meetsMinimum: true,
  }),
  scriptPath: vi.fn().mockResolvedValue({ path: "/tmp/demo/deploy/x.sql" }),
  scriptRead: vi.fn().mockResolvedValue({ content: "-- sql", path: "x", error: null }),
  onStatusStale: noop,
  onWatchEvent: noop,
  onSqitchStream: noop,
